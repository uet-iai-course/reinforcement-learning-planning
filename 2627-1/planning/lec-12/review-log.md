...
## Chuẩn hóa tiền tố ánh xạ — 04-09-2026

- Kiểm định toàn khóa phát hiện deck dùng mã rút gọn `topic-01`–`topic-15`, trong khi lecture note dùng `lec-12-topic-01`–`lec-12-topic-15`. Đã chuẩn hóa 50 thuộc tính `data-note-topic-id` và hai bảng ánh xạ trong outline/storyboard; không đổi nội dung hiển thị, công thức, thứ tự, `data-slide-id`, ghi chú hoặc SVG.
- Writer GLM dùng `patch/5/300/5000`, reasoning `minimal`, đã ghi đủ thay đổi nhưng kết thúc nguyên văn với `model exceeded the tool-call limit (5)` trước khi trả báo cáo. Lượt này không được tính là worker hoàn tất; diff và số lượng được điều phối viên kiểm trực tiếp.
- Tái rà chuyên môn bằng DeepSeek V4 Flash dùng `recheck/6/300/6000`, reasoning `none`, hoàn tất ở vòng 6 và PASS. Runtime trả `requested_model=observed_model=deepseek/deepseek-v4-flash-0731`, provider `OpenRouter`.
- Tái rà mạch GLM lần đầu dừng nguyên văn với `model exceeded the tool-call limit (6)` do tự tìm HTML sai thư mục. Chạy lại cùng model với bốn đường dẫn khóa sẵn và `recheck/8/300/6000`; hoàn tất ở vòng 3 và PASS. Runtime trả `requested_model=observed_model=z-ai/glm-5.3-flash`, provider `OpenRouter`, reasoning `minimal`.
- Kết quả cuối: 50 `data-slide-id` duy nhất, mỗi trang có đúng một mã thuộc tập 15 `note-topic-id`, đủ cả 15 mã; outline và storyboard khớp HTML. Không còn `data-note-topic-id="topic-NN"`.
- Chromium dựng lại đủ 50 trang ở 1280×720 và 800×600: không lỗi KaTeX, console, tài nguyên, bàn phím hoặc phần tử vượt khung trang hiện tại.
- Lượt kiểm toàn khóa chạy 48 ca trình duyệt: 12 deck và 12 lecture note, mỗi loại ở hai kích thước. Cả 487 trang chiếu và 24 lượt dựng note đạt; không có lỗi runtime, tài nguyên hỏng, tràn thân tài liệu hoặc marker HTML hiển thị. Kiểm định tĩnh đồng thời xác nhận 12 thẻ index, 12 cặp liên kết, 36 tệp planning, 94 SVG hợp lệ XML và không có `quill.json`.
- Bốn Design Files HTML, outline, storyboard và review-log đã được ghi lại vào dự án Codex Slides rồi đọc lại khớp từng byte. Codex in-editor Browser vẫn không có trong phiên, nên không tuyên bố đã rà trực quan bằng giao diện đó.
