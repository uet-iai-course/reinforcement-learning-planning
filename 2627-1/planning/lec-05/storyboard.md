...
| TD(0) | B00 | B00,B05 | B00,B03–B04 | B01–B02,B06–B07 | B03–B05 | B08 | chuyển mẫu → cập nhật → kỳ vọng Bellman → hội tụ | 32 |
...
- A00 cho lượt $e_1$ trước công thức A01, nêu chính sách dự định Right và xác suất đảo chiều $0{,}2$. A02 nêu đủ đầu vào, khởi tạo, vòng lượt, đầu ra, dừng, chi phí và quy tắc mỗi trạng thái tối đa một mẫu trong mỗi lượt của MC lần ghé đầu.
...
- A06 tách mọi lần ghé khỏi tuyến lõi; nhãn hàng alpha ghi rõ đây là MC mọi lần ghé, không phải TD. A07 chỉ phát biểu hội tụ chính cho lần ghé đầu với các lượt khởi động lại độc lập.
- B00 cho chuyển $S\to x$ trước ký hiệu. B02 nêu đủ đầu vào, khởi tạo, vòng chuyển, đầu ra, dừng và chi phí của TD(0). B03–B04 dùng cập nhật tại chỗ; B05 thuộc topic 09 và so thời điểm cập nhật trước khi B06 lấy kỳ vọng.
...
- C00–C01 dùng đi bộ dài; C00 nêu ba ô không tên giữa $S$ và $G$, bốn chuyển tới $G$, hai chuyển tới $L$, và không dùng hai giá trị chuẩn của nguồn vì không tái tạo nhất quán từ mô hình đã nêu. C02 báo rõ trở lại lượt ngắn, chốt $\gamma=1$, $\alpha=0{,}5$ và bảng khởi tạo bằng không; nguồn số liệu tr. 29. C03 nối số nguồn ngẫu nhiên trong $G_t$ với biến động, rồi nối phần đuôi $V_t$ với chệch.
...
| B07 | Robbins–Monro dùng số lần cập nhật của từng trạng thái; tổng alpha phân kỳ để tiếp tục điều chỉnh, tổng bình phương hữu hạn để nhiễu không tích lũy vô hạn; nhánh $\gamma=1$ cần lượt khởi động lại và $\pi$ đúng đắn — đi tới kết thúc với xác suất $1$ từ mọi trạng thái có thể đạt được từ phân phối khởi tạo dưới $\pi$, các trạng thái không kết thúc là quá độ. | Kiểm cập nhật tại chỗ. |
...
| D00 | Năm phân biệt cần giữ, gồm so sánh chệch–phương sai có điều kiện và không có xếp hạng phổ quát. | Nối bài sau. |
...
| X03 | Tính đích, sai số và cập nhật TD với $\alpha_{n(s)}(s)=0{,}1$. | Chữa ngắn. |
...
- Sau rà deck theo note, B05 được thêm vào ánh xạ topic 09; C00 làm rõ số ô và số chuyển; B07 tách điều kiện $\gamma=1$ khỏi định nghĩa chính sách đúng đắn để giảm tải mà không lược giả thiết.
